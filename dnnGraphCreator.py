...
from models.code_models.torch_models import DenseNN
...
def evaluate_metrics(y_true, y_pred):
    # Keep everything on GPU using PyTorch operations
    rmse = torch.sqrt(torch.mean((y_true - y_pred) ** 2, dim=0))
    mae = torch.mean(torch.abs(y_true - y_pred), dim=0)
    r2 = 1 - torch.sum((y_true - y_pred) ** 2, dim=0) / torch.sum((y_true - torch.mean(y_true, dim=0)) ** 2, dim=0)
    mape = torch.mean(torch.abs((y_true - y_pred) / (y_true + 1e-6)), dim=0) * 100
    
    # Only move to CPU and convert to numpy for final results
    return {
        "rmse": rmse.cpu().numpy(), 
        "mae": mae.cpu().numpy(), 
        "r2": r2.cpu().numpy(), 
...
def automl_torch_densenn(approach,
                          batch_size=1028,
    ...
    print(f"\n{'='*60}")
    print(f"Starting training with approach: {approach}")
    ...
    for i in [1, 2, 3, 4, 5, 6]:
        ...
        # MODEL
        # ===============================
        model = DenseNN(input_dim=X_train_full.shape[1], hidden_dims=hidden_dims, dropout=dropout).to(device)
        ...
        # TRAIN
        # ===============================
        print(f"Training iteration {i}/6: {iterations[-1]}")
        model.train()
        pbar = tqdm(range(epochs), desc=f"Epoch", ncols=100)
        for epoch in pbar:
            epoch_loss = 0
            ...
            pbar.set_postfix({'loss': f'{epoch_loss:.4f}', 'best': f'{best_loss:.4f}', 'patience': f'{patience_counter}/{patience}'})
            ...
            if epoch_loss < best_loss:
                ...
                if patience_counter >= patience:
                    ...
                    pbar.set_description(f"Epoch (stopped early)")
                    break
...
automl_torch_densenn("accumulative")
automl_torch_densenn("pairs")
automl_torch_densenn("future-testing")
for a in ['accumulative', 'pairs', 'future-testing']:
    automl_torch_densenn(a)